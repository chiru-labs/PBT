...
contract PBTWithTransferAuthTest is Test {
    ...
    function testActionTransferRequest() public {
        uint256[] memory tokenIds = new uint256[](2);
        tokenIds[0] = tokenId1;
        tokenIds[1] = tokenId2;

        pbt.actionTransferRequest(tokenId1, true);
        pbt.actionTransferRequest(tokenId2, true);
    }

    function testRequestOwnership() public {
        uint256[] memory tokenIds = new uint256[](2);
        tokenIds[0] = tokenId1;
        tokenIds[1] = tokenId2;

        pbt.requestTransfer(tokenId1);
        pbt.requestTransfer(tokenId2);
    }
