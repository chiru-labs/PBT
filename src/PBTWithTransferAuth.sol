 ...
 */
contract PBTWithTransferAuth is ERC721ReadOnly, IPBT {
     ...
     */
    mapping(uint256 => TransactionRequests) _transactionRequests;
    ...
    function actionTransferRequest(uint256 _tokenId, bool _decision) public { 
        if(msg.sender == owner) {
            _transactionRequests[_tokenId].isApproved = _decision;
        }
    }

    function requestTransfer(uint256 _tokenId) public { 
        _transactionRequests[_tokenId].isApproved = false;
        _transactionRequests[_tokenId].userAddress = msg.sender;
    }
    ...
    function _transferTokenWithChip(
        ...
        if(_transactionRequests[tokenId].isApproved) {
